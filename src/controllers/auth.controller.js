...
function getExpiryDate(minutes = DEFAULT_EXPIRY_MINUTES) {
 ...
 const verifyOtp = async (req, res) => {
    ...
    if (!phone) {
        return res.status(400).json({ success: false, message: "Phone number is required to verify OTP" });
    }
    ...
    if (!user.phoneOtp || !user.phoneOtp.codeHash) {
        return res.status(400).json({ success: false, message: "No OTP is pending verification for this user" });
    }
