...
exports.sendNotificationCore = async ({ userId, title, bodi, data, topic, imageUrl, silent , highPriority , androidChannelId = "high_importance_channel",
    ...
    if (!title || !bodi) {
        throw { status: 400, error: 'Missing required fields', required: ['title', 'bodi'] };
    }
    let user=undefined;
    if(userId){
   ...
   message.apns = {
    ...
    return { success: true, messageId: response, message: 'Notification sent successfully to user' };
}
